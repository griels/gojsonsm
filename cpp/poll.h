...
std::tuple<int, int, int, Sockaddr, moku::error> ReadMsg(moku::slice<uint8_t> p, moku::slice<uint8_t> oob);
...
std::tuple<int, int, moku::error> WriteMsg(moku::slice<uint8_t> p, moku::slice<uint8_t> oob, Sockaddr sa);
...
moku::error SetsockoptInet4Addr(int level, int name, std::vector<uint8_t> arg);
...
std::tuple<int64_t, moku::error> Writev(moku::slice<moku::slice<uint8_t>>* v);
};
template <> inline bool moku::is_nil<FD>(const FD& fd) {return moku::is_nil<fdMutex>(fd.fdmu)&&moku::is_nil<int>(fd.Sysfd)&&moku::is_nil<pollDesc>(fd.pd)&&moku::is_nil<moku::slice<Iovec>*>(fd.iovecs)&&moku::is_nil<bool>(fd.IsStream)&&moku::is_nil<bool>(fd.ZeroReadIsEOF)&&moku::is_nil<bool>(fd.isFile); }uintptr_t PollDescriptor();
std::function<void(int)> TestHookDidWritev;

struct TimeoutError{ std::string Error();
bool Timeout();
bool Temporary();
};
template <> inline bool moku::is_nil<TimeoutError>(const TimeoutError& timeouterror) {return ; }std::tuple<int, Sockaddr, std::string, moku::error> accept(int s);
...
struct fdMutex{ uint64_t state{0};
uint32_t rsema{0};
uint32_t wsema{0};bool incref();
bool increfAndClose();
bool decref();
bool rwlock(bool read);
bool rwunlock(bool read);
};
template <> inline bool moku::is_nil<fdMutex>(const fdMutex& fdmutex) {return moku::is_nil<uint64_t>(fdmutex.state)&&moku::is_nil<uint32_t>(fdmutex.rsema)&&moku::is_nil<uint32_t>(fdmutex.wsema); }static constexpr int maxRW{1073741824};
...
struct pollDesc{ uintptr_t runtimeCtx{0};moku::error init(FD* fd);
void close();
void evict();
...
template <> inline bool moku::is_nil<pollDesc>(const pollDesc& polldesc) {return moku::is_nil<uintptr_t>(polldesc.runtimeCtx); }int64_t runtimeNano();
