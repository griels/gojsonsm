...
{
if (isFile) {return ErrFileClosing;
...
{
while (len(*v) > 0){int64_t ln0{0};

ln0 = int64_t(len((*v)[0]));
if (ln0 > n) {(*v)[0] = moku::slice_expr<moku::slice<uint8_t>>((*v)[0], n);
return ;
}n -= ln0;
...
bool fdMutex::incref()
{
while (true){uint64_t new{0};
uint64_t old{0};

old = atomic::LoadUint64(&this->state);
if (old & mutexClosed != 0) {return false;
}new = old + mutexRef;
if (new & mutexRefMask == 0) {panic(moku::make_iface<std::string>(&{%!s(token.Pos=4397900) STRING "inconsistent poll.fdMutex"}));
}if (atomic::CompareAndSwapUint64(&this->state, old, new)) {return true;
}
}
...
if (new & mutexRefMask == 0) {panic(moku::make_iface<std::string>(&{%!s(token.Pos=4398378) STRING "inconsistent poll.fdMutex"}));
}new &= ~(mutexRMask | mutexWMask);
if (atomic::CompareAndSwapUint64(&this->state, old, new)) {while (old & mutexRMask != 0){
old -= mutexRWait;
...
old = atomic::LoadUint64(&this->state);
if (old & mutexRefMask == 0) {panic(moku::make_iface<std::string>(&{%!s(token.Pos=4399047) STRING "inconsistent poll.fdMutex"}));
}new = old - mutexRef;
if (atomic::CompareAndSwapUint64(&this->state, old, new)) {return new & (mutexClosed | mutexRefMask) == mutexClosed;
}
}
...
if (read) {mutexBit = mutexRLock;
mutexWait = mutexRWait;
mutexMask = mutexRMask;
mutexSema = &this->rsema;
} else {mutexBit = mutexWLock;
...
old = atomic::LoadUint64(&this->state);
if (old & mutexClosed != 0) {return false;
}if (old & mutexBit == 0) {new = (old | mutexBit) + mutexRef;
if (new & mutexRefMask == 0) {panic(moku::make_iface<std::string>(&{%!s(token.Pos=4399907) STRING "inconsistent poll.fdMutex"}));
}} else {new = old + mutexWait;
if (new & mutexMask == 0) {panic(moku::make_iface<std::string>(&{%!s(token.Pos=4400035) STRING "inconsistent poll.fdMutex"}));
}}if (atomic::CompareAndSwapUint64(&this->state, old, new)) {if (old & mutexBit == 0) {return true;
...
old = atomic::LoadUint64(&this->state);
if (old & mutexBit == 0 || old & mutexRefMask == 0) {panic(moku::make_iface<std::string>(&{%!s(token.Pos=4400810) STRING "inconsistent poll.fdMutex"}));
}new = (old & ~(mutexBit)) - mutexRef;
if (old & mutexMask != 0) {new -= mutexWait;
}if (atomic::CompareAndSwapUint64(&this->state, old, new)) {if (old & mutexMask != 0) {runtime_Semrelease(mutexSema);
...
{
if (!this->fdmu.incref()) {return errClosing(this->isFile);
}return nil;

}
moku::error FD::decref()
{
if (this->fdmu.decref()) {return this->destroy();
}return nil;

}
moku::error FD::readLock()
{
if (!this->fdmu.rwlock(true)) {return errClosing(this->isFile);
}return nil;

}
void FD::readUnlock()
{
if (this->fdmu.rwunlock(true)) {this->destroy();
}
}
moku::error FD::writeLock()
{
if (!this->fdmu.rwlock(false)) {return errClosing(this->isFile);
}return nil;

}
void FD::writeUnlock()
{
if (this->fdmu.rwunlock(false)) {this->destroy();
}
}
void init();
int main() {
...
if (errno != 0) {if (ctx != 0) {runtime_pollUnblock(ctx);
...
if (this->runtimeCtx == 0) {return nil;
...
if (this->runtimeCtx == 0) {return errors::New("waiting for unsupported file type");
...
if ((res == 0)) {_ident_0_:
return nil;
}else if ((res == 1)) {_ident_1_:
return errClosing(isFile);
}else if ((res == 2)) {_ident_2_:
return ErrTimeout;
}println("unreachable: ", res);
panic(moku::make_iface<std::string>(&{%!s(token.Pos=4394880) STRING "unreachable"}));
...
}}if (fd->pd.runtimeCtx == 0) {return errors::New("file type does not support deadlines");
...
{
if (n == 0 && moku::is_nil(err) && this->ZeroReadIsEOF) {return io::EOF;
}return err;
...
{
if (net == "file") {this->isFile = true;
}if (!pollable) {return nil;
...
moku::error FD::Close()
{
if (!this->fdmu.increfAndClose()) {return errClosing(this->isFile);
}this->pd.evict();
return this->decref();
...
if (!moku::is_nil(err)) {return {0, err};
}}if (this->IsStream && len(p) > maxRW) {p = moku::slice_expr<moku::slice<uint8_t>>(p, maxRW);
}while (true){moku::error err{};
int n{0};

std::tie(n, err) = syscall::Read(this->Sysfd, p);
if (!moku::is_nil(err)) {n = 0;
if (err == syscall::EAGAIN && this->pd.pollable()) {{
err = this->pd.waitRead(this->isFile);

if (moku::is_nil(err)) {continue;
...
if (!moku::is_nil(err)) {return {0, nil, err};
}}_defer_.Push([=]() mutable {this->readUnlock(); });
{moku::error err{};
...
std::tuple<int, int, int, Sockaddr, moku::error> FD::ReadMsg(moku::slice<uint8_t> p, moku::slice<uint8_t> oob)
{moku::defer _defer_;
...
if (!moku::is_nil(err)) {return {0, 0, 0, nil, err};
}}_defer_.Push([=]() mutable {this->readUnlock(); });
{moku::error err{};
...
std::tie(n, oobn, flags, sa, err) = syscall::Recvmsg(this->Sysfd, p, oob, 0);
if (!moku::is_nil(err)) {if (err == syscall::EAGAIN && this->pd.pollable()) {{
err = this->pd.waitRead(this->isFile);
...
max = len(p);
if (this->IsStream && max - nn > maxRW) {max = nn + maxRW;
}std::tie(n, err) = syscall::Write(this->Sysfd, moku::slice_expr<moku::slice<uint8_t>>(p, nn, max));
if (n > 0) {nn += n;
}if (nn == len(p)) {return {nn, err};
}if (err == syscall::EAGAIN && this->pd.pollable()) {{
err = this->pd.waitWrite(this->isFile);

if (moku::is_nil(err)) {continue;
}}}if (!moku::is_nil(err)) {return {nn, err};
}if (n == 0) {return {nn, io::ErrUnexpectedEOF};
}
}
...
max = len(p);
if (this->IsStream && max - nn > maxRW) {max = nn + maxRW;
}std::tie(n, err) = syscall::Pwrite(this->Sysfd, moku::slice_expr<moku::slice<uint8_t>>(p, nn, max), off + int64_t(nn));
if (n > 0) {nn += n;
}if (nn == len(p)) {return {nn, err};
}if (!moku::is_nil(err)) {return {nn, err};
}if (n == 0) {return {nn, io::ErrUnexpectedEOF};
...
std::tuple<int, int, moku::error> FD::WriteMsg(moku::slice<uint8_t> p, moku::slice<uint8_t> oob, Sockaddr sa)
{moku::defer _defer_;
...
if (!moku::is_nil(err)) {return {0, 0, err};
}}_defer_.Push([=]() mutable {this->writeUnlock(); });
{moku::error err{};
...
if (moku::is_nil(err)) {continue;
}}}if (!moku::is_nil(err)) {return {n, 0, err};
...
if (!moku::is_nil(err)) {return {-1, nil, "", err};
}}_defer_.Push([=]() mutable {this->readUnlock(); });
{moku::error err{};
...
if (moku::is_nil(err)) {return {s, rsa, "", err};
}if ((err == syscall::EAGAIN)) {_ident_0_:
if (this->pd.pollable()) {{
err = this->pd.waitRead(this->isFile);

if (moku::is_nil(err)) {continue;
}}}}else if ((err == syscall::ECONNABORTED)) {_ident_1_:
continue;
...
if (f(uintptr_t(this->Sysfd))) {return nil;
}{moku::error err{};
...
moku::error FD::SetsockoptInet4Addr(int level, int name, std::vector<uint8_t> arg)
{moku::defer _defer_;
...
if (moku::is_nil(err)) {syscall::CloseOnExec(ns);
}if (!moku::is_nil(err)) {return {-1, nil, "accept", err};
}{
err = syscall::SetNonblock(ns, true);

if (!moku::is_nil(err)) {CloseFunc(ns);
...
std::tuple<int64_t, moku::error> FD::Writev(moku::slice<moku::slice<uint8_t>>* v)
{moku::defer _defer_;
moku::error err{};
...
if (!moku::is_nil(err)) {return {0, err};
}}if (!moku::is_nil(this->iovecs)) {iovecs = *this->iovecs;
}maxVec = 1024;
while (len(*v) > 0){Errno e0{};
uintptr_t wrote{0};

iovecs = moku::slice_expr<moku::slice<Iovec>>(iovecs, 0);
{moku::slice<uint8_t> chunk{};
for (chunk : moku::range_value<moku::slice<moku::slice<uint8_t>>>(*v)) {if (len(chunk) == 0) {continue;
}iovecs = append(iovecs, syscall::Iovec{{Base, &chunk[0]}});
if (this->IsStream && len(chunk) > 1 << 30) {iovecs[len(iovecs) - 1].SetLen(1 << 30);
break;
}iovecs[len(iovecs) - 1].SetLen(len(chunk));
if (len(iovecs) == maxVec) {break;
}}
}if (len(iovecs) == 0) {break;
}this->iovecs = &iovecs;
std::tie(wrote, _, e0) = syscall::Syscall(syscall::SYS_WRITEV, uintptr_t(this->Sysfd), uintptr_t(unsafe::Pointer(&iovecs[0])), uintptr_t(len(iovecs)));
if (wrote == ^uintptr_t(0)) {wrote = 0;
...
if (moku::is_nil(err)) {continue;
}}} else {if (e0 != 0) {err = syscall::Errno(e0);
