...
std::tuple<moku::slice<uint8_t>, moku::error> PrivateKey::Sign(Reader rand, moku::slice<uint8_t> msg, SignerOpts opts)
{moku::error err{};
Int* r{std::nullptr};
Int* s{std::nullptr};
...
}return asn1::Marshal(moku::make_iface<ecdsaSignature>(&{ecdsaSignature %!s(token.Pos=6273402) [r s] %!s(token.Pos=6273407)}));
...
priv->D = k;
std::tie(priv->PublicKey.X, priv->PublicKey.Y) = c.ScalarBaseMult(k->Bytes());
return {priv, nil};
...
if (len(hash) > orderBytes) {hash = moku::slice_expr<moku::slice<uint8_t>>(hash, orderBytes);
}ret = new(big::Int)->SetBytes(hash);
excess = len(hash) * 8 - orderBits;
if (excess > 0) {ret->Rsh(ret, unsigned int(excess));
}return ret;
...
std::tuple<Int*, Int*, moku::error> Sign(Reader rand, PrivateKey* priv, moku::slice<uint8_t> hash)
{Int* N{std::nullptr};
Block block{};
Curve c{};
StreamReader csprng{};
...
if (entropylen > 32) {entropylen = 32;
...
if (!moku::is_nil(err)) {return {nil, nil, err};
}csprng = cipher::StreamReader{R: zeroReader, S: cipher::NewCTR(block, moku::slice<byte>(aesIV))};
c = priv->PublicKey.Curve;
N = c.Params()->N;
if (N->Sign() == 0) {return {nil, nil, errZeroParam};
}while (true){Int* e{std::nullptr};

while (true){
std::tie(k, err) = randFieldElement(c, csprng);
if (!moku::is_nil(err)) {r = nil;
return ;
}{invertible in{};
bool ok{false};
...
if (ok) {kInv = in.Inverse(k);
} else {kInv = fermatInverse(k, N);
...
if (ok) {w = in.Inverse(s);
} else {w = new(big::Int)->ModInverse(s, N);
}}u1 = e->Mul(e, w);
u1->Mod(u1, N);
u2 = w->Mul(r, w);
u2->Mod(u2, N);
...
if (ok) {std::tie(x, y) = opt.CombinedMult(pub->X, pub->Y, u1->Bytes(), u2->Bytes());
} else {std::tie(x1, y1) = c.ScalarBaseMult(u1->Bytes());
std::tie(x2, y2) = c.ScalarMult(pub->X, pub->Y, u2->Bytes());
std::tie(x, y) = c.Add(x1, y1, x2, y2);
}}if (x->Sign() == 0 && y->Sign() == 0) {return false;
}x->Mod(x, N);
return x->Cmp(r) == 0;
...
{int i{0};
for (i : moku::range_key<moku::slice<uint8_t>>(dst)) {dst[i] = 0;
}
}return {len(dst), nil};
