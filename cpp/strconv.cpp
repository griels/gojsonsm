...
{
if ((str == "1") || (str == "t") || (str == "T") || (str == "true") || (str == "TRUE") || (str == "True")) {_ident_0_:
return {true, nil};
}else if ((str == "0") || (str == "f") || (str == "F") || (str == "false") || (str == "FALSE") || (str == "False")) {_ident_1_:
return {false, nil};
...
for (i = 0; i < len(s1); i++){uint8_t c1{0};
uint8_t c2{0};
...
if (equalIgnoreCase(s, "+inf") || equalIgnoreCase(s, "+infinity")) {return {math::Inf(1), true};
}}else if ((s[0] == '-')) {_ident_2_:
if (equalIgnoreCase(s, "-inf") || equalIgnoreCase(s, "-infinity")) {return {math::Inf(-1), true};
}}else if ((s[0] == 'n') || (s[0] == 'N')) {_ident_3_:
if (equalIgnoreCase(s, "nan")) {return {math::NaN(), true};
}}else if ((s[0] == 'i') || (s[0] == 'I')) {_ident_4_:
if (equalIgnoreCase(s, "inf") || equalIgnoreCase(s, "infinity")) {return {math::Inf(1), true};
}}else {_ident_0_:
return ;
}return ;
...
this->dp = this->nd;
continue;
}else if (('0' <= s[i] && s[i] <= '9')) {_ident_8_:
sawdigits = true;
if (s[i] == '0' && this->nd == 0) {this->dp--;
continue;
}if (this->nd < len(this->d)) {this->d[this->nd] = s[i];
this->nd++;
} else {if (s[i] != '0') {this->trunc = true;
}}continue;
}break;

}
if (!sawdigits) {return ;
}if (!sawdot) {this->dp = this->nd;
}if (i < len(s) && (s[i] == 'e' || s[i] == 'E')) {i++;
if (i >= len(s)) {return ;
}esign = 1;
if (s[i] == '+') {i++;
} else {if (s[i] == '-') {i++;
...
for (; i < len(s); i++){
{uint8_t c{0};
c = s[i];if ((true == c == '.')) {_ident_11_:
if (sawdot) {return ;
}sawdot = true;
dp = nd;
continue;
}else if ((true == '0' <= c && c <= '9')) {_ident_12_:
sawdigits = true;
if (c == '0' && nd == 0) {dp--;
continue;
}nd++;
...
exp = flt->bias;
goto out;
}if (this->dp > 310) {goto overflow;
}if (this->dp < -330) {mant = 0;
exp = flt->bias;
goto out;
}exp = 0;
while (this->dp > 0){int n{0};

if (this->dp >= len(powtab)) {n = 27;
} else {n = powtab[this->dp];
}this->Shift(-n);
exp += n;

}
while (this->dp < 0 || this->dp == 0 && this->d[0] < '5'){int n{0};
...
exp--;
if (exp < flt->bias + 1) {n = flt->bias + 1 - exp;
this->Shift(-n);
exp += n;
}if (exp - flt->bias >= 1 << flt->expbits - 1) {goto overflow;
}this->Shift(int(1 + flt->mantbits));
mant = this->RoundedInteger();
if (mant == 2 << flt->mantbits) {mant >>= 1;
exp++;
if (exp - flt->bias >= 1 << flt->expbits - 1) {goto overflow;
}}if (mant & (1 << flt->mantbits) == 0) {exp = flt->bias;
}goto out;
overflow:
;
...
if (this->neg) {bits |= 1 << flt->mantbits << flt->expbits;
...
if (mantissa >> float64info.mantbits != 0) {return ;
}f = double(mantissa);
if (neg) {f = -f;
}if ((exp == 0)) {_ident_13_:
return {f, true};
}else if ((exp > 0 && exp <= 15 + 22)) {_ident_14_:
if (exp > 22) {f *= float64pow10[exp - 22];
exp = 22;
}if (f > 1e15 || f < -1e15) {return ;
}return {f * float64pow10[exp], true};
}else if ((exp < 0 && exp >= -22)) {_ident_15_:
return {f / float64pow10[-exp], true};
}return ;
...
if (mantissa >> float32info.mantbits != 0) {return ;
}f = float(mantissa);
if (neg) {f = -f;
}if ((exp == 0)) {_ident_16_:
return {f, true};
}else if ((exp > 0 && exp <= 7 + 10)) {_ident_17_:
if (exp > 10) {f *= float32pow10[exp - 10];
exp = 10;
}if (f > 1e7 || f < -1e7) {return ;
}return {f * float32pow10[exp], true};
}else if ((exp < 0 && exp >= -10)) {_ident_18_:
return {f / float32pow10[-exp], true};
}return ;
...
if (ok) {return {float(val), nil};
}}if (optimize) {std::tie(mantissa, exp, neg, trunc, ok) = readFloat(s);
if (ok) {if (!trunc) {{float f{0};
bool ok{false};
...
ok = ext->AssignDecimal(mantissa, exp, neg, trunc, &float32info);

if (ok) {std::tie(b, ovf) = ext->floatBits(&float32info);
f = math::Float32frombits(uint32_t(b));
if (ovf) {err = rangeError(fnParseFloat, s);
}return {f, err};
}}}}if (!d.set(s)) {return {0, syntaxError(fnParseFloat, s)};
...
ok = ext->AssignDecimal(mantissa, exp, neg, trunc, &float64info);

if (ok) {std::tie(b, ovf) = ext->floatBits(&float64info);
...
{
if (bitSize == 32) {std::tie(f, err) = atof32(s);
return {double(f), err};
}return atof64(s);
...
{
return "strconv." + this->Func + ": " + "parsing " + Quote(this->Num) + ": " + this->Err.Error();
...
std::tuple<uint64_t, moku::error> ParseUint(std::string s, int base, int bitSize)
{uint64_t cutoff{0};
moku::error err{};
int i{0};
uint64_t maxVal{0};
...
if (bitSize == 0) {bitSize = int(IntSize);
}i = 0;
if ((len(s) < 1)) {_ident_0_:
err = ErrSyntax;
goto Error;
}else if ((2 <= base && base <= 36)) {_ident_1_:
}else if ((base == 0)) {_ident_2_:
if ((s[0] == '0' && len(s) > 1 && (s[1] == 'x' || s[1] == 'X'))) {_ident_4_:
if (len(s) < 3) {err = ErrSyntax;
goto Error;
}base = 16;
i = 2;
}else if ((s[0] == '0')) {_ident_5_:
base = 8;
i = 1;
}else {_ident_6_:
base = 10;
...
goto Error;
}if ((base == 10)) {_ident_7_:
cutoff = maxUint64 / 10 + 1;
}else if ((base == 16)) {_ident_8_:
cutoff = maxUint64 / 16 + 1;
}else {_ident_9_:
cutoff = maxUint64 / uint64_t(base) + 1;
}maxVal = 1 << unsigned int(bitSize) - 1;
for (; i < len(s); i++){uint8_t d{0};
uint64_t n1{0};
uint8_t v{0};

d = s[i];
if (('0' <= d && d <= '9')) {_ident_10_:
v = d - '0';
}else if (('a' <= d && d <= 'z')) {_ident_11_:
v = d - 'a' + 10;
}else if (('A' <= d && d <= 'Z')) {_ident_12_:
v = d - 'A' + 10;
}else {_ident_13_:
n = 0;
err = ErrSyntax;
...
err = ErrSyntax;
goto Error;
}if (n >= cutoff) {n = maxUint64;
err = ErrRange;
goto Error;
}n *= uint64_t(base);
n1 = n + uint64_t(v);
if (n1 < n || n1 > maxVal) {n = maxUint64;
err = ErrRange;
goto Error;
}n = n1;
...
if (bitSize == 0) {bitSize = int(IntSize);
}if (len(s) == 0) {return {0, syntaxError(fnParseInt, s)};
}s0 = s;
neg = false;
if (s[0] == '+') {s = moku::slice_expr<std::string>(s, 1);
} else {if (s[0] == '-') {neg = true;
s = moku::slice_expr<std::string>(s, 1);
}}std::tie(un, err) = ParseUint(s, base, bitSize);
if (!moku::is_nil(err) && moku::try_assert<*NumError>(err)->Err != ErrRange) {moku::try_assert<*NumError>(err)->Func = fnParseInt;
moku::type_assert<*NumError>(err)->Num = s0;
return {0, err};
}cutoff = uint64_t(1 << unsigned int(bitSize - 1));
if (!neg && un >= cutoff) {return {int64_t(cutoff - 1), rangeError(fnParseInt, s0)};
}if (neg && un > cutoff) {return {-int64_t(cutoff), rangeError(fnParseInt, s0)};
}n = int64_t(un);
if (neg) {n = -n;
}return {n, nil};
...
n = 10 + this->nd;
if (this->dp > 0) {n += this->dp;
}if (this->dp < 0) {n += -this->dp;
}buf = make(moku::slice<byte>, n);
w = 0;
if ((this->nd == 0)) {_ident_0_:
return "0";
}else if ((this->dp <= 0)) {_ident_1_:
buf[w] = '0';
w++;
buf[w] = '.';
w++;
w += digitZero(moku::slice_expr<moku::slice<uint8_t>>(buf, w, w + -this->dp));
w += copy(moku::slice_expr<moku::slice<uint8_t>>(buf, w), moku::slice_expr<std::vector<uint8_t>>(this->d, 0, this->nd));
}else if ((this->dp < this->nd)) {_ident_2_:
w += copy(moku::slice_expr<moku::slice<uint8_t>>(buf, w), moku::slice_expr<std::vector<uint8_t>>(this->d, 0, this->dp));
buf[w] = '.';
w++;
w += copy(moku::slice_expr<moku::slice<uint8_t>>(buf, w), moku::slice_expr<std::vector<uint8_t>>(this->d, this->dp, this->nd));
}else {_ident_3_:
w += copy(moku::slice_expr<moku::slice<uint8_t>>(buf, w), moku::slice_expr<std::vector<uint8_t>>(this->d, 0, this->nd));
w += digitZero(moku::slice_expr<moku::slice<uint8_t>>(buf, w, w + this->dp - this->nd));
...
r = 0;
w = 0;
for (; n >> k == 0; r++){unsigned int c{0};

if (r >= a->nd) {if (n == 0) {a->nd = 0;
return ;
}while (n >> k == 0){
n = n * 10;
r++;
...
while (n > 0){unsigned int dig{0};

dig = n >> k;
n &= mask;
if (w < len(a->d)) {a->d[w] = byte(dig + '0');
w++;
} else {if (dig > 0) {a->trunc = true;
}}n = n * 10;
...
if (prefixIsLessThan(moku::slice_expr<std::vector<uint8_t>>(a->d, 0, a->nd), leftcheats[k].cutoff)) {delta--;
}r = a->nd;
w = a->nd + delta;
for (r--; r >= 0; r--){unsigned int quo{0};
unsigned int rem{0};
...
rem = n - 10 * quo;
w--;
if (w < len(a->d)) {a->d[w] = byte(rem + '0');
} else {if (rem != 0) {a->trunc = true;
}}n = quo;

}
while (n > 0){unsigned int quo{0};
unsigned int rem{0};
...
{
if ((this->nd == 0)) {_ident_4_:
}else if ((k > 0)) {_ident_5_:
while (k > maxShift){
...
{
if (nd < 0 || nd >= a->nd) {return false;
}if (a->d[nd] == '5' && nd + 1 == a->nd) {if (a->trunc) {return true;
...
{
if (nd < 0 || nd >= this->nd) {return ;
}if (shouldRoundUp(this, nd)) {this->RoundUp(nd);
} else {this->RoundDown(nd);
}
}
...
if (this->dp > 20) {return 0xFFFFFFFFFFFFFFFF;
}n = uint64_t(0);
...
if (shouldRoundUp(this, this->dp)) {n++;
}return n;
...
if (this->mant & (1 << (62 - flt->mantbits)) != 0) {mant += 1;
}if (mant == 2 << flt->mantbits) {mant >>= 1;
exp++;
}if (exp - flt->bias >= 1 << flt->expbits - 1) {mant = 0;
exp = 1 << flt->expbits - 1 + flt->bias;
overflow = true;
} else {if (mant & (1 << flt->mantbits) == 0) {exp = flt->bias;
...
std::tuple<extFloat, extFloat> extFloat::AssignComputeBounds(uint64_t mant, int exp, bool neg, floatInfo* flt)
{int expBiased{0};
extFloat lower{};
extFloat upper{};
...
this->neg = neg;
if (this->exp <= 0 && mant == (mant >> unsigned int(-this->exp)) << unsigned int(-this->exp)) {this->mant >>= unsigned int(-this->exp);
this->exp = 0;
return {*this, *this};
}expBiased = exp - flt->bias;
upper = extFloat{mant: 2 * this->mant + 1, exp: this->exp - 1, neg: this->neg};
if (mant != 1 << flt->mantbits || expBiased == 1) {lower = extFloat{mant: 2 * this->mant - 1, exp: this->exp - 1, neg: this->neg};
} else {lower = extFloat{mant: 4 * this->mant - 1, exp: this->exp - 2, neg: this->neg};
}return ;
...
std::tie(fhi, flo) = std::tuple<uint64_t, uint64_t>(this->mant >> 32, uint64_t(uint32_t(this->mant)));
std::tie(ghi, glo) = std::tuple<uint64_t, uint64_t>(g.mant >> 32, uint64_t(uint32_t(g.mant)));
cross1 = fhi * glo;
cross2 = flo * ghi;
this->mant = fhi * ghi + (cross1 >> 32) + (cross2 >> 32);
rem = uint64_t(uint32_t(cross1)) + uint64_t(uint32_t(cross2)) + ((flo * glo) >> 32);
rem += (1 << 31);
this->mant += (rem >> 32);
this->exp = this->exp + g.exp + 64;

}
bool extFloat::AssignDecimal(uint64_t mantissa, int exp10, bool neg, bool trunc, floatInfo* flt)
{int adjExp{0};
int denormalExp{0};
int errors{0};
...
errors = 0;
if (trunc) {errors += errorscale / 2;
}this->mant = mantissa;
this->exp = 0;
this->neg = neg;
i = (exp10 - firstPowerOfTen) / stepPowerOfTen;
if (exp10 < firstPowerOfTen || i >= len(powersOfTen)) {return false;
}adjExp = (exp10 - firstPowerOfTen) % stepPowerOfTen;
if (adjExp < uint64digits && mantissa < uint64pow10[uint64digits - adjExp]) {this->mant *= uint64pow10[adjExp];
this->Normalize();
} else {this->Normalize();
...
if (this->exp <= denormalExp) {extrabits = 63 - flt->mantbits + 1 + unsigned int(denormalExp - this->exp);
} else {extrabits = 63 - flt->mantbits;
}halfway = uint64_t(1) << (extrabits - 1);
mant_extra = this->mant & (1 << extrabits - 1);
if (int64_t(halfway) - int64_t(errors) < int64_t(mant_extra) && int64_t(mant_extra) < int64_t(halfway) + int64_t(errors)) {return false;
}return true;
...
if (this->mant == 0) {d->nd = 0;
d->dp = 0;
d->neg = this->neg;
return true;
}if (n == 0) {panic(moku::make_iface<std::string>(&{%!s(token.Pos=1383787) STRING "strconv: internal error: extFloat.FixedDecimal called with n == 0"}));
}this->Normalize();
...
{int i{0};
uint64_t pow{0};
for (std::tie(i, pow) = std::tuple<int, uint64_t>(0, uint64_t(1)); i < 20; i++){
if (pow > uint64_t(integer)) {integerDigits = i;
break;
}pow *= 10;

}
}rest = integer;
if (integerDigits > needed) {pow10 = uint64pow10[integerDigits - needed];
integer *= uint32_t(pow10);
rest -= integer * uint32_t(pow10);
} else {rest = 0;
}pos = len(buf);
{uint32_t v{0};
for (v = integer; v > 0; ){uint32_t v1{0};
...
needed -= nd;
if (needed > 0) {if (rest != 0 || pow10 != 1) {panic(moku::make_iface<std::string>(&{%!s(token.Pos=1385147) STRING "strconv: internal error, rest != 0 but needed > 0"}));
}while (needed > 0){uint64_t digit{0};

fraction *= 10;
ε *= 10;
if (2 * ε > 1 << shift) {return false;
...
d->nd = nd;
}ok = adjustLastDigitFixed(d, uint64_t(rest) << shift | fraction, pow10, shift, ε);
if (!ok) {return false;
}{int i{0};
for (i = d->nd - 1; i >= 0; i--){
if (d->d[i] != '0') {d->nd = i + 1;
...
bool adjustLastDigitFixed(decimalSlice* d, uint64_t num, uint64_t den, unsigned int shift, uint64_t ε)
{
if (num > den << shift) {panic(moku::make_iface<std::string>(&{%!s(token.Pos=1387104) STRING "strconv: num > den<<shift in adjustLastDigitFixed"}));
}if (2 * ε > den << shift) {panic(moku::make_iface<std::string>(&{%!s(token.Pos=1387192) STRING "strconv: ε > (den<<shift)/2"}));
}if (2 * (num + ε) < den << shift) {return true;
}if (2 * (num - ε) > den << shift) {i = d->nd - 1;
for (; i >= 0; i--){
if (d->d[i] == '9') {d->nd--;
} else {break;
}
...
bool extFloat::ShortestDecimal(decimalSlice* d, extFloat* lower, extFloat* upper)
...
d->neg = this->neg;
return true;
}if (this->exp == 0 && *lower == *this && *lower == *upper) {n = len(buf) - 1;
{uint64_t v{0};
for (v = this->mant; v > 0; ){uint64_t v1{0};
...
if (this->exp > upper->exp) {this->mant <<= unsigned int(this->exp - upper->exp);
this->exp = upper->exp;
}if (lower->exp > upper->exp) {lower->mant <<= unsigned int(lower->exp - upper->exp);
lower->exp = upper->exp;
...
for (i = 0; i < integerDigits; i++){uint32_t digit{0};
uint64_t pow{0};
...
if (currentDiff < allowance) {d->nd = i + 1;
d->dp = integerDigits + exp10;
d->neg = this->neg;
...
if (fraction < allowance * multiplier) {return adjustLastDigit(d, fraction, targetDiff * multiplier, allowance * multiplier, 1 << shift, multiplier * 2);
}
}

}
bool adjustLastDigit(decimalSlice* d, uint64_t currentDiff, uint64_t targetDiff, uint64_t maxDiff, uint64_t ulpDecimal, uint64_t ulpBinary)
{
if (ulpDecimal < 2 * ulpBinary) {return false;
}while (currentDiff + ulpDecimal / 2 + ulpBinary < targetDiff){
d->d[d->nd - 1]--;
currentDiff += ulpDecimal;

}
if (currentDiff + ulpDecimal <= targetDiff + ulpDecimal / 2 + ulpBinary) {return false;
}if (currentDiff < ulpBinary || currentDiff > maxDiff - ulpBinary) {return false;
}if (d->nd == 1 && d->d[0] == '0') {d->nd = 0;
d->dp = 0;
}return true;
...
{
return std::string(genericFtoa(make(moku::slice<byte>, 0, max(prec + 4, 24)), f, fmt, prec, bitSize));

}
moku::slice<uint8_t> AppendFloat(moku::slice<uint8_t> dst, double f, uint8_t fmt, int prec, int bitSize)
{
return genericFtoa(dst, f, fmt, prec, bitSize);

}
moku::slice<uint8_t> genericFtoa(moku::slice<uint8_t> dst, double val, uint8_t fmt, int prec, int bitSize)
{uint64_t bits{0};
decimalSlice digs{};
int exp{0};
...
if ((bitSize == 32)) {_ident_0_:
bits = uint64_t(math::Float32bits(float(val)));
flt = &float32info;
}else if ((bitSize == 64)) {_ident_1_:
bits = math::Float64bits(val);
flt = &float64info;
}else {_ident_2_:
panic(moku::make_iface<std::string>(&{%!s(token.Pos=1418947) STRING "strconv: illegal AppendFloat/FormatFloat bitSize"}));
}neg = bits >> (flt->expbits + flt->mantbits) != 0;
exp = int(bits >> flt->mantbits) & (1 << flt->expbits - 1);
mant = bits & (uint64_t(1) << flt->mantbits - 1);
if ((exp == 1 << flt->expbits - 1)) {_ident_3_:
if ((mant != 0)) {_ident_6_:
s = "NaN";
}else if ((neg)) {_ident_7_:
s = "-Inf";
}else {_ident_8_:
s = "+Inf";
}return append(dst, s);
}else if ((exp == 0)) {_ident_4_:
exp++;
}else {_ident_5_:
mant |= uint64_t(1) << flt->mantbits;
}exp += flt->bias;
if (fmt == 'b') {return fmtB(dst, neg, mant, exp, flt);
}if (!optimize) {return bigFtoa(dst, prec, fmt, neg, mant, exp, flt);
}ok = false;
shortest = prec < 0;
if (shortest) {f = new(extFloat);
std::tie(lower, upper) = f->AssignComputeBounds(mant, exp, neg, flt);
digs.d = moku::slice_expr<std::vector<uint8_t>>(buf);
ok = f->ShortestDecimal(&digs, &lower, &upper);
if (!ok) {return bigFtoa(dst, prec, fmt, neg, mant, exp, flt);
}if ((fmt == 'e') || (fmt == 'E')) {_ident_9_:
prec = max(digs.nd - 1, 0);
}else if ((fmt == 'f')) {_ident_10_:
prec = max(digs.nd - digs.dp, 0);
}else if ((fmt == 'g') || (fmt == 'G')) {_ident_11_:
prec = digs.nd;
}} else {if (fmt != 'f') {digits = prec;
if ((fmt == 'e') || (fmt == 'E')) {_ident_12_:
digits++;
}else if ((fmt == 'g') || (fmt == 'G')) {_ident_13_:
if (prec == 0) {prec = 1;
}digits = prec;
}if (digits <= 15) {digs.d = moku::slice_expr<std::vector<uint8_t>>(buf);
...
moku::slice<uint8_t> bigFtoa(moku::slice<uint8_t> dst, int prec, uint8_t fmt, bool neg, uint64_t mant, int exp, floatInfo* flt)
...
shortest = prec < 0;
if (shortest) {roundShortest(d, mant, exp, flt);
digs = decimalSlice{d: moku::slice_expr<std::vector<uint8_t>>(d->d), nd: d->nd, dp: d->dp};
if ((fmt == 'e') || (fmt == 'E')) {_ident_14_:
prec = digs.nd - 1;
}else if ((fmt == 'f')) {_ident_15_:
prec = max(digs.nd - digs.dp, 0);
}else if ((fmt == 'g') || (fmt == 'G')) {_ident_16_:
prec = digs.nd;
}} else {if ((fmt == 'e') || (fmt == 'E')) {_ident_17_:
d->Round(prec + 1);
}else if ((fmt == 'f')) {_ident_18_:
d->Round(d->dp + prec);
}else if ((fmt == 'g') || (fmt == 'G')) {_ident_19_:
if (prec == 0) {prec = 1;
}d->Round(prec);
}digs = decimalSlice{d: moku::slice_expr<std::vector<uint8_t>>(d->d), nd: d->nd, dp: d->dp};
}return formatDigits(dst, shortest, neg, digs, prec, fmt);
...
moku::slice<uint8_t> formatDigits(moku::slice<uint8_t> dst, bool shortest, bool neg, decimalSlice digs, int prec, uint8_t fmt)
{
if ((fmt == 'e') || (fmt == 'E')) {_ident_20_:
return fmtE(dst, neg, digs, prec, fmt);
}else if ((fmt == 'f')) {_ident_21_:
return fmtF(dst, neg, digs, prec);
}else if ((fmt == 'g') || (fmt == 'G')) {_ident_22_:
eprec = prec;
if (eprec > digs.nd && digs.nd >= digs.dp) {eprec = digs.nd;
}if (shortest) {eprec = 6;
}exp = digs.dp - 1;
if (exp < -4 || exp >= eprec) {if (prec > digs.nd) {prec = digs.nd;
...
if (exp > minexp && 332 * (d->dp - d->nd) >= 100 * (exp - int(flt->mantbits))) {return ;
}upper = new(decimal);
upper->Assign(mant * 2 + 1);
upper->Shift(exp - int(flt->mantbits) - 1);
if (mant > 1 << flt->mantbits || exp == minexp) {mantlo = mant - 1;
explo = exp;
} else {mantlo = mant * 2 - 1;
explo = exp - 1;
}lower = new(decimal);
...
l = byte('0');
if (i < lower->nd) {l = lower->d[i];
}m = d->d[i];
u = byte('0');
if (i < upper->nd) {u = upper->d[i];
}okdown = l != m || inclusive && i + 1 == lower->nd;
okup = m != u && (inclusive || m + 1 < u || i + 1 < upper->nd);
if ((okdown && okup)) {_ident_23_:
d->Round(i + 1);
return ;
}else if ((okdown)) {_ident_24_:
d->RoundDown(i + 1);
return ;
}else if ((okup)) {_ident_25_:
d->RoundUp(i + 1);
return ;
}
}
...
moku::slice<uint8_t> fmtE(moku::slice<uint8_t> dst, bool neg, decimalSlice d, int prec, uint8_t fmt)
{uint8_t ch{0};
int exp{0};

if (neg) {dst = append(dst, '-');
}ch = byte('0');
if (d.nd != 0) {ch = d.d[0];
}dst = append(dst, ch);
if (prec > 0) {dst = append(dst, '.');
i = 1;
m = min(d.nd, prec + 1);
if (i < m) {dst = append(dst, moku::slice_expr<moku::slice<uint8_t>>(d.d, i, m));
i = m;
}for (; i <= prec; i++){
dst = append(dst, '0');
...
if ((exp < 10)) {_ident_26_:
dst = append(dst, '0', byte(exp) + '0');
}else if ((exp < 100)) {_ident_27_:
...
moku::slice<uint8_t> fmtF(moku::slice<uint8_t> dst, bool neg, decimalSlice d, int prec)
{
if (neg) {dst = append(dst, '-');
}if (d.dp > 0) {m = min(d.nd, d.dp);
dst = append(dst, moku::slice_expr<moku::slice<uint8_t>>(d.d, m));
for (; m < d.dp; m++){
dst = append(dst, '0');

}
} else {dst = append(dst, '0');
}if (prec > 0) {dst = append(dst, '.');
{int i{0};
for (i = 0; i < prec; i++){uint8_t ch{0};
...
moku::slice<uint8_t> fmtB(moku::slice<uint8_t> dst, bool neg, uint64_t mant, int exp, floatInfo* flt)
{
if (neg) {dst = append(dst, '-');
...
if (fastSmalls && i < nSmalls && base == 10) {return small(int(i));
...
{
if (fastSmalls && 0 <= i && i < nSmalls && base == 10) {return append(dst, small(int(i)));
...
{
if (fastSmalls && i < nSmalls && base == 10) {return append(dst, small(int(i)));
...
std::tuple<moku::slice<uint8_t>, std::string> formatBits(moku::slice<uint8_t> dst, uint64_t u, int base, bool neg, bool append_)
{std::vector<uint8_t> a{};
moku::slice<uint8_t> d{};
int i{0};
std::string s{""};

if (base < 2 || base > len(digits)) {panic(moku::make_iface<std::string>(&{%!s(token.Pos=1430794) STRING "strconv: illegal AppendInt/FormatInt base"}));
}i = len(a);
if (neg) {u = -u;
}if (base == 10) {if (host32bit) {while (u >= 1e9){uint64_t q{0};
unsigned int us{0};

q = u / 1e9;
us = unsigned int(u - q * 1e9);
{int j{0};
for (j = 4; j > 0; j--){unsigned int is{0};
...
while (us >= 100){unsigned int is{0};
...
if (us >= 10) {i--;
a[i] = smallsString[is];
}} else {{unsigned int s{0};

s = shifts[base];

if (s > 0) {b = uint64_t(base);
...
} else {b = uint64_t(base);
while (u >= b){uint64_t q{0};
...
a[i] = '-';
}if (append_) {d = append(dst, moku::slice_expr<std::vector<uint8_t>>(a, i));
return ;
...
std::string quoteWith(std::string s, uint8_t quote, bool ASCIIonly, bool graphicOnly)
{
return std::string(appendQuotedWith(make(moku::slice<byte>, 0, 3 * len(s) / 2), s, quote, ASCIIonly, graphicOnly));

}
std::string quoteRuneWith(int32_t r, uint8_t quote, bool ASCIIonly, bool graphicOnly)
{
return std::string(appendQuotedRuneWith(nil, r, quote, ASCIIonly, graphicOnly));

}
moku::slice<uint8_t> appendQuotedWith(moku::slice<uint8_t> buf, std::string s, uint8_t quote, bool ASCIIonly, bool graphicOnly)
{
buf = append(buf, quote);
{int width{0};
for (width = 0; len(s) > 0; s = moku::slice_expr<std::string>(s, width)){int32_t r{0};

r = rune(s[0]);
width = 1;
if (r >= utf8::RuneSelf) {std::tie(r, width) = utf8::DecodeRuneInString(s);
}if (width == 1 && r == utf8::RuneError) {buf = append(buf, `\x`);
...
moku::slice<uint8_t> appendQuotedRuneWith(moku::slice<uint8_t> buf, int32_t r, uint8_t quote, bool ASCIIonly, bool graphicOnly)
{
buf = append(buf, quote);
...
moku::slice<uint8_t> appendEscapedRune(moku::slice<uint8_t> buf, int32_t r, uint8_t quote, bool ASCIIonly, bool graphicOnly)
{std::vector<uint8_t> runeTmp{};

if (r == rune(quote) || r == '\\') {buf = append(buf, '\\');
buf = append(buf, byte(r));
return buf;
}if (ASCIIonly) {if (r < utf8::RuneSelf && IsPrint(r)) {buf = append(buf, byte(r));
return buf;
}} else {if (IsPrint(r) || graphicOnly && isInGraphicList(r)) {n = utf8::EncodeRune(moku::slice_expr<std::vector<uint8_t>>(runeTmp), r);
buf = append(buf, moku::slice_expr<std::vector<uint8_t>>(runeTmp, n));
return buf;
}}if ((r == '\a')) {_ident_0_:
buf = append(buf, `\a`);
}else if ((r == '\b')) {_ident_1_:
buf = append(buf, `\b`);
}else if ((r == '\f')) {_ident_2_:
buf = append(buf, `\f`);
}else if ((r == '\n')) {_ident_3_:
buf = append(buf, `\n`);
}else if ((r == '\r')) {_ident_4_:
buf = append(buf, `\r`);
}else if ((r == '\t')) {_ident_5_:
buf = append(buf, `\t`);
}else if ((r == '\v')) {_ident_6_:
buf = append(buf, `\v`);
}else {_ident_7_:
if ((r < ' ')) {_ident_8_:
buf = append(buf, `\x`);
...
}else if ((r > utf8::MaxRune)) {_ident_9_:
r = 0xFFFD;
goto _ident_10_;
}else if ((r < 0x10000)) {_ident_10_:
buf = append(buf, `\u`);
{int s{0};
for (s = 12; s >= 0; s -= 4){
...
moku::slice<uint8_t> AppendQuoteToGraphic(moku::slice<uint8_t> dst, std::string s)
{
...
moku::slice<uint8_t> AppendQuoteRuneToGraphic(moku::slice<uint8_t> dst, int32_t r)
{
...
{
while (len(s) > 0){int32_t r{0};
int wid{0};

std::tie(r, wid) = utf8::DecodeRuneInString(s);
s = moku::slice_expr<std::string>(s, wid);
if (wid > 1) {if (r == '\ufeff') {return false;
}continue;
}if (r == utf8::RuneError) {return false;
}if ((r < ' ' && r != '\t') || r == '`' || r == '\u007F') {return false;
}
}
return true;
...
c = rune(b);
if (('0' <= c && c <= '9')) {_ident_12_:
return {c - '0', true};
}else if (('a' <= c && c <= 'f')) {_ident_13_:
return {c - 'a' + 10, true};
}else if (('A' <= c && c <= 'F')) {_ident_14_:
return {c - 'A' + 10, true};
}return ;

}
std::tuple<int32_t, bool, std::string, moku::error> UnquoteChar(std::string s, uint8_t quote)
{uint8_t c{0};
moku::error err{};
...
{uint8_t c{0};
c = s[0];if ((c == quote && (quote == '\'' || quote == '"'))) {_ident_15_:
err = ErrSyntax;
return ;
}else if ((c >= utf8::RuneSelf)) {_ident_16_:
std::tie(r, size) = utf8::DecodeRuneInString(s);
return {r, true, moku::slice_expr<std::string>(s, size), nil};
}else if ((c != '\\')) {_ident_17_:
return {rune(s[0]), false, moku::slice_expr<std::string>(s, 1), nil};
}}if (len(s) <= 1) {err = ErrSyntax;
return ;
}c = s[1];
s = moku::slice_expr<std::string>(s, 2);
if ((c == 'a')) {_ident_18_:
value = '\a';
}else if ((c == 'b')) {_ident_19_:
value = '\b';
}else if ((c == 'f')) {_ident_20_:
value = '\f';
}else if ((c == 'n')) {_ident_21_:
value = '\n';
}else if ((c == 'r')) {_ident_22_:
value = '\r';
}else if ((c == 't')) {_ident_23_:
value = '\t';
}else if ((c == 'v')) {_ident_24_:
value = '\v';
}else if ((c == 'x') || (c == 'u') || (c == 'U')) {_ident_25_:
n = 0;
if ((c == 'x')) {_ident_30_:
n = 2;
}else if ((c == 'u')) {_ident_31_:
n = 4;
}else if ((c == 'U')) {_ident_32_:
n = 8;
}if (len(s) < n) {err = ErrSyntax;
return ;
}{int j{0};
for (j = 0; j < n; j++){bool ok{false};
int32_t x{0};
...
if (c == 'x') {value = v;
break;
}if (v > utf8::MaxRune) {err = ErrSyntax;
return ;
}value = v;
multibyte = true;
}else if ((c == '0') || (c == '1') || (c == '2') || (c == '3') || (c == '4') || (c == '5') || (c == '6') || (c == '7')) {_ident_26_:
v = rune(c) - '0';
if (len(s) < 2) {err = ErrSyntax;
return ;
}{int j{0};
for (j = 0; j < 2; j++){int32_t x{0};

x = rune(s[j]) - '0';
if (x < 0 || x > 7) {err = ErrSyntax;
return ;
}v = (v << 3) | x;

}
}s = moku::slice_expr<std::string>(s, 2);
if (v > 255) {err = ErrSyntax;
return ;
}value = v;
}else if ((c == '\\')) {_ident_27_:
value = '\\';
}else if ((c == '\'') || (c == '"')) {_ident_28_:
if (c != quote) {err = ErrSyntax;
return ;
}value = rune(c);
}else {_ident_29_:
err = ErrSyntax;
...
if (quote != s[n - 1]) {return {"", ErrSyntax};
}s = moku::slice_expr<std::string>(s, 1, n - 1);
if (quote == '`') {if (contains(s, '`')) {return {"", ErrSyntax};
}if (contains(s, '\r')) {buf = make(moku::slice<byte>, 0, len(s) - 1);
{int i{0};
for (i = 0; i < len(s); i++){
if (s[i] != '\r') {buf = append(buf, s[i]);
}
}
}return {std::string(buf), nil};
}return {s, nil};
}if (quote != '"' && quote != '\'') {return {"", ErrSyntax};
}if (contains(s, '\n')) {return {"", ErrSyntax};
}if (!contains(s, '\\') && !contains(s, quote)) {if ((quote == '"')) {_ident_33_:
return {s, nil};
}else if ((quote == '\'')) {_ident_34_:
std::tie(r, size) = utf8::DecodeRuneInString(s);
if (size == len(s) && (r != utf8::RuneError || size != 1)) {return {s, nil};
}}}buf = make(moku::slice<byte>, 0, 3 * len(s) / 2);
while (len(s) > 0){int32_t c{0};
moku::error err{};
bool multibyte{false};
std::string ss{""};
...
if (c < utf8::RuneSelf || !multibyte) {buf = append(buf, byte(c));
} else {n = utf8::EncodeRune(moku::slice_expr<std::vector<uint8_t>>(runeTmp), c);
buf = append(buf, moku::slice_expr<std::vector<uint8_t>>(runeTmp, n));
}if (quote == '\'' && len(s) != 0) {return {"", ErrSyntax};
}
}
return {std::string(buf), nil};
...
if (r <= 0xFF) {if (0x20 <= r && r <= 0x7E) {return true;
}if (0xA1 <= r && r <= 0xFF) {return r != 0xAD;
}return false;
}if (0 <= r && r < 1 << 16) {std::tie(rr, isPrint, isNotPrint) = std::tuple<uint16_t, moku::slice<uint16_t>, moku::slice<uint16_t>>(uint16_t(r), isPrint16, isNotPrint16);
i = bsearch16(isPrint, rr);
if (i >= len(isPrint) || rr < isPrint[i & ~(1)] || isPrint[i | 1] < rr) {return false;
}j = bsearch16(isNotPrint, rr);
return j >= len(isNotPrint) || isNotPrint[j] != rr;
}std::tie(rr, isPrint, isNotPrint) = std::tuple<uint32_t, moku::slice<uint32_t>, moku::slice<uint16_t>>(uint32_t(r), isPrint32, isNotPrint32);
