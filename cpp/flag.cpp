...
i = 0;
{Flag* f{std::nullptr};
for (f : moku::range_value<std::map<std::string, Flag*>>(flags)) {list[i] = f->Name;
i++;
}
}list.Sort();
...
{int i{0};
std::string name{""};
for (std::tie(i, name) : moku::range_key_value<StringSlice>(list)) {result[i] = flags[name];
}
}return result;
...
Writer FlagSet::out()
{
if (moku::is_nil(this->output)) {return os::Stderr;
}return this->output;
...
for (flag : moku::range_value<moku::slice<Flag*>>(sortFlags(this->formal))) {fn(flag);
}
}
}
...
{
{Flag* flag{std::nullptr};
for (flag : moku::range_value<moku::slice<Flag*>>(sortFlags(this->actual))) {fn(flag);
}
}
}
...
if (!ok) {return fmt::Errorf("no such flag -%v", name);
}err = flag::Value.Set(value);
if (!moku::is_nil(err)) {return err;
}if (moku::is_nil(this->actual)) {this->actual = make(std::map<std::string, *Flag>);
...
}if (value == moku::type_assert<Value>(z.Interface()).String()) {return true;
}if ((value == "false")) {_ident_0_:
return true;
}else if ((value == "")) {_ident_1_:
return true;
}else if ((value == "0")) {_ident_2_:
return true;
}return false;
