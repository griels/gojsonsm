...
moku::slice<uint8_t> AppendReverse(moku::slice<uint8_t> out, moku::slice<uint8_t> in);
constexpr Class B{7};
...
template <> inline bool moku::is_nil<Class>(const Class& class) {return moku::is_nil<unsigned int>(unsigned int(class)); }inline bool in(moku::slice<Class> set) {
Class class = *this;
...
template <> inline bool moku::is_nil<Direction>(const Direction& direction) {return moku::is_nil<int>(int(direction)); }constexpr Class EN{2};
