...
struct WordDecoder{ std::function<Reader, moku::error(std::string, Reader)> CharsetReader{std::nullptr};std::tuple<std::string, moku::error> Decode(std::string word);
std::tuple<std::string, moku::error> DecodeHeader(std::string header);
moku::error convert(Buffer* buf, std::string charset, moku::slice<uint8_t> content);
};
template <> inline bool moku::is_nil<WordDecoder>(const WordDecoder& worddecoder) {return moku::is_nil<std::function<Reader, moku::error(std::string, Reader)>>(worddecoder.CharsetReader); }
struct WordEncoder: public moku::basic<uint8_t> {
WordEncoder() : moku::basic<uint8_t>{0} {}
};
template <> inline bool moku::is_nil<WordEncoder>(const WordEncoder& wordencoder) {return moku::is_nil<uint8_t>(uint8_t(wordencoder)); }inline std::string Encode(std::string charset, std::string s) {
...
WordEncoder wordencoder = *this;
wordencoder._bEncodeByValue(Buffer* buf, std::string charset, std::string s);
}
...
WordEncoder wordencoder = *this;
wordencoder._qEncodeByValue(Buffer* buf, std::string charset, std::string s);
}
