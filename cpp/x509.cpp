...
{
return &CertPool{{bySubjectKeyId, make(std::map<std::string, moku::slice<int>>)}, {byName, make(std::map<std::string, moku::slice<int>>)}};

}
std::tuple<CertPool*, moku::error> SystemCertPool()
{
if (runtime::GOOS == "windows") {return {nil, errors::New("crypto/x509: system root pool is not available on Windows")};
}return loadSystemRoots();

}
std::tuple<moku::slice<int>, Certificate*, moku::error> CertPool::findVerifiedParents(Certificate* cert)
{moku::slice<int> candidates{};
moku::error err{};
Certificate* errCert{std::nullptr};
moku::slice<int> parents{};

if (moku::is_nil(this)) {return ;
}if (len(cert->AuthorityKeyId) > 0) {candidates = this->bySubjectKeyId[std::string(cert->AuthorityKeyId)];
}if (len(candidates) == 0) {candidates = this->byName[std::string(cert->RawIssuer)];
}{int c{0};
for (c : moku::range_value<moku::slice<int>>(candidates)) {{
err = cert->CheckSignatureFrom(this->certs[c]);

if (moku::is_nil(err)) {parents = append(parents, c);
} else {errCert = this->certs[c];
}}}
}return ;
...
if (moku::is_nil(this)) {return false;
}candidates = this->byName[std::string(cert->RawSubject)];
{int c{0};
for (c : moku::range_value<moku::slice<int>>(candidates)) {if (this->certs[c]->Equal(cert)) {return true;
}}
}return false;
...
if (moku::is_nil(cert)) {panic(moku::make_iface<std::string>(&{%!s(token.Pos=4966498) STRING "adding nil Certificate to CertPool"}));
}if (this->contains(cert)) {return ;
}n = len(this->certs);
this->certs = append(this->certs, cert);
if (len(cert->SubjectKeyId) > 0) {keyId = std::string(cert->SubjectKeyId);
...
while (len(pemCerts) > 0){Block* block{std::nullptr};
Certificate* cert{std::nullptr};
moku::error err{};

std::tie(block, pemCerts) = pem::Decode(pemCerts);
if (moku::is_nil(block)) {break;
...
algoName= std::vector<std::string>{{MD2WithRSA, "MD2-RSA"}, {MD5WithRSA, "MD5-RSA"}, {SHA1WithRSA, "SHA1-RSA"}, {SHA256WithRSA, "SHA256-RSA"}, {SHA384WithRSA, "SHA384-RSA"}, {SHA512WithRSA, "SHA512-RSA"}, {SHA256WithRSAPSS, "SHA256-RSAPSS"}, {SHA384WithRSAPSS, "SHA384-RSAPSS"}, {SHA512WithRSAPSS, "SHA512-RSAPSS"}, {DSAWithSHA1, "DSA-SHA1"}, {DSAWithSHA256, "DSA-SHA256"}, {ECDSAWithSHA1, "ECDSA-SHA1"}, {ECDSAWithSHA256, "ECDSA-SHA256"}, {ECDSAWithSHA384, "ECDSA-SHA384"}, {ECDSAWithSHA512, "ECDSA-SHA512"}};
